...
if ($tipo_usuario == "1"){
	...
	if($res->num_rows > 0){
		...
		while ($row = $res->fetch_object()) {
			...
			print "<td>
					  <button onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=dashboard&pag=atendente-salvar&acao=excluir&id_usuario=".$row->id_usuario."';}else{false;}\"  class='btn btn-outline-danger'>Excluir</button>
				   </td>";
			print "</tr>";
		}
		print "</table>";
	...
	// Verificação para usuários diferentes de administradores
